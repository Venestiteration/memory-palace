#!/usr/bin/env python3
"""
capture_voice.py - 语音转录文本捕获

接收语音转录文本，写入 00_Inbox/voice/。
支持 CLI 和 API 两种调用方式。

注意：此脚本不处理音频文件，只接收 transcript 文本输入。
音频处理（如 Whisper 转录）由外部流程完成。

用法：
  python capture_voice.py <transcript_text> [title]
  echo "<transcript>" | python capture_voice.py --stdin [title]
"""
...
def write_capture(result: CaptureResult, dry_run: bool = False) -> dict:
    ...
    Returns:
        JSON 结果字典
    """
    output_dir = ensure_inbox_dir("voice")
...
def main():
    ...
    if "--stdin" in sys.argv:
        ...
        title = sys.argv[1] if len(sys.argv) > 1 else "Voice Note"
    elif len(sys.argv) < 2:
        print(json.dumps({
            "success": False,
            "error": "用法: python capture_voice.py <transcript> [title]\n       echo <transcript> | python capture_voice.py --stdin [title]"
        }, ensure_ascii=False, indent=2))
        sys.exit(2)
    else:
        content = sys.argv[1]
        title = sys.argv[2] if len(sys.argv) > 2 else "Voice Note"
