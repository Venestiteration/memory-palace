...
执行：index → vector → daily brief

用法:
  python run_daily.py              # 执行全流程
  python run_daily.py --dry-run    # 预览模式（不写文件）
"""
...
def run_daily(dry_run: bool = False, logger: logging.Logger = None) -> int:
    """执行每日例行任务"""
    load_env()
    ...
    if logger is None:
        ...
        logger = setup_logging("run_daily", log_file)

    logger.info("=" * 50)
    logger.info("开始每日例行任务")
    logger.info("=" * 50)

    # 记录开始状态
    start_time = time.time()
    update_job_status("daily", JobStatus.RUNNING)
    ...
    try:
        ...
        update_job_status("daily", JobStatus.RUNNING, steps=steps)
        ...
        # Step 3: generate_daily_brief.py
        step_name = "generate_daily_brief"
        logger.info(f"步骤 3/3: 生成每日简报")
        update_job_status("daily", JobStatus.RUNNING, steps=steps)
        cmd = [sys.executable, str(SCRIPT_DIR / "generate_daily_brief.py")]
        if dry_run:
            cmd.append("--dry-run")
        ret, stdout, stderr = run_command(cmd, logger)
        if ret != 0:
            error_msg = stderr or stdout or "generate_daily_brief 失败"
            steps.append({"name": step_name, "status": "failure", "error": error_msg[:200]})
        ...
        update_job_status("daily", JobStatus.SUCCESS, duration_seconds=duration, steps=steps)
        logger.info("每日例行任务完成")

    except Exception as e:
        duration = time.time() - start_time
        error_str = str(e)
        diagnostic = get_diagnostic_info("daily", error_str, overall_exit_code)
...
def main():
    parser = argparse.ArgumentParser(description="每日例行任务")
    ...
    ret = run_daily(dry_run=args.dry_run, logger=logger)
    sys.exit(ret)
